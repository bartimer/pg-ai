...
const colors = ["#7284B7", "#E67C78", "#EBB77C", "#A9A8C4", "#FFD9C3"]
const layout = {
  width: 300,
  height: 200,
  margin: {
...
export function SensorPage() {
  const { id } = useParams()
  const { isLoading, error, data: sensor } = useQuery(`sensors/${id}`, () => getSensor(id))
  const { isLoading: isLoadingData, error: errorData, data } = useQuery(`sensors/${sensor?.name}`, () => getLastSensorDataFor(sensor?.name),
    { enabled: !!sensor })
  let { isLoading: isLoadingPredictionsData, error: errorPredictionsData, data: predictions } = useQuery(`sensors-predictions/${sensor?.name}`, () => getPredictionsFor(sensor?.name),
    { enabled: !!sensor })
